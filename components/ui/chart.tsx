    ...
    >
      {formatter && item?.value !== undefined && item.name ? (
        ...
        <>
          ...
          >
            <div className="grid gap-1.5 flex-row">
            ...
            {item.value != null && (
              <span className="font-medium font-mono text-white tabular-nums ml-2">
...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    if (labelFormatter) {
      return (
        <div className={cn("font-bold text-white !text-white", labelClassName)}>
    ...
    return <div className={cn("font-bold text-white !text-white", labelClassName)}>{value}</div>;
  }, [
    label,
    labelFormatter,
    payload,
  ...
  return (
    <MotionDiv
      animate={{ opacity: 1 }}
      className={cn(
        "grid min-w-32 items-start gap-1.5 rounded-lg border border-white/10 bg-black/95 p-3 text-xs shadow-xl text-white !text-white [&_*]:!text-white",
        className
      )}
      style={{ color: '#ffffff' }}
    ...
    >
      ...
      <div className="grid gap-1.5 text-white !text-white [&_*]:!text-white">
        {payload
    ...
    >
      {payload
        ...
        .map((item, index) => {
          ...
          return (
            <div
              className="flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-white/60"
              key={index}
            >
